(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/framework/manager/ResManager.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '6e438mtQJFAHqoIBocbM9Mz', 'ResManager', __filename);
// script/framework/manager/ResManager.js
...
var ResManager = {
    ...
    isSameURL: function isSameURL(src, dest) {
        return !!src && !!dest && src.dir === dest.dir && src.name === dest.name;
    },

    getIsCloseByOther: function getIsCloseByOther(url) {
        for (var key in this.prefabURL) {
    ...
    //获得炮的动画路径
    //index 是基础炮多少管
    getWeaponURLByLevel: function getWeaponURLByLevel(level) {
    ...
    //获得自己当前炮的形象
    getSelfWeaponURL: function getSelfWeaponURL() {
        var url = null;
    ...
    getEffectBgType: function getEffectBgType(type) {
        var str = type && type == 2 ? '_baozha' : '_kaihuo';
        var effect = null;
        // if (PlayerData.armAvatar > WeaponHelper.jichuValue) {
    ...
    //@countUint 艺术字 特殊符号
    //@ CongraNode 恭喜获得
    setCommonItemData: function setCommonItemData(iteminfo, itemIcon, nameLabel, countLabel, descLabel, merge, uint, countUint, CongraNode, special) {
        if (iteminfo) {
            var items = iteminfo.split('|');
            if (items.length > 0) {
                var itemConf = ConfigManager.dataMap.Item;
                for (var j = 0; j < itemConf.length; j++) {
                    if (itemConf[j].itemid === parseInt(items[1])) {
                        if (cc.isValid(itemIcon)) {
                            SpriteManager.setSprite(itemIcon, 'icon', itemConf[j].name);
                        }
                        var itemName = i18n.t(itemConf[j].nameLanguage);
                        if (!!merge && merge === true) {
    ...
    //设置背包中的数据
    setBagItemData: function setBagItemData(iteminfo, itemIcon, nameLabel, countLabel, descLabel) {
        if (iteminfo) {
            var items = iteminfo;
            var itemConf = void 0;
            if (items.length > 0) {
    ...
    //设置道具图标
    setCommonIcon: function setCommonIcon(iteminfo, itemIcon) {
        if (iteminfo) {
            var items = iteminfo.split('|');
            if (items.length > 0) {
                var itemConf = ConfigManager.dataMap.Item;
    ...
    //获得道具的名称
    getCommonItemName: function getCommonItemName(iteminfo, isNotCount) {
        if (iteminfo) {
            var items = iteminfo.split('|');
            if (items.length > 0) {
                var itemConf = ConfigManager.dataMap.Item;
    ...
    //获得道具的名称
    getCommonItemNameByID: function getCommonItemNameByID(itemId) {
        if (itemId) {
            var itemConf = ConfigManager.dataMap.Item;
            for (var j = 0; j < itemConf.length; j++) {
    ...
    setCommonItemData2: function setCommonItemData2(iteminfo, itemIcon, nameLabel, countLabel, merge, countUint) {
        if (iteminfo) {
            var items = iteminfo.split('|');
            if (items.length > 0) {
                var itemConf = ConfigManager.dataMap.Item;
